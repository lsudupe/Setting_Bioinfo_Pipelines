...
trend_top <- topTable(fit, coef=ncol(design))
...
fit <- lmFit(v, design)
fit <- eBayes(fit)
voom_top <-topTable(fit, coef=ncol(design))
...
trend_all <- topTable(fit, coef=ncol(design), number = nrow(dge), sort.by = "none")
...
Correlation between both results suggest perfect linear agreement. For each gene, the log-fold change estimated by voom is exactly proportional to the log-fold change estimated by trend.
